...
class RunEventArtifacts:
    """Handle per-run output directory, event logging, and task provenance."""
    ...
    def attach_event_file_writer(self, metrics_logger) -> None:
        """Monkeypatch WorkforceLogger._log_event to emit per-event .txt files."""
    ...
    def attach_agent_message_event_bridge(self, metrics_logger) -> None:
        """Bridge camel logging Agent Message output into WorkforceLogger events."""
    ...
    def attach_loguru_tool_event_bridge(self, metrics_logger) -> None:
        """Bridge loguru tool logs into WorkforceLogger events."""
        def _sink(message):
